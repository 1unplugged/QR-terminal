...
def get_qr(request, key):
    ...
    my_url = 'http://unplugged.pythonanywhere.com/check/'
...
def check(request, key):
    ...
    try:
        ...
        send_message(
            f'Попытка использовать неверный код:  {key}'
            f'\nДата попытки: {str(dt.datetime.now())[:-7]}'
        )
    ...
    message = (
            ...
            '\nДоступ получен в: ' + str(dt.datetime.now())[:-7]
    )
